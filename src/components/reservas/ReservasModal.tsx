...
export default function ReservasModal({ isOpen, onClose, propiedadId, propiedadTitulo }: ReservasModalProps) {
    ...
    useEffect(() => {
        if (fechaEntrada) {
            ...
            const minDate = entrada.toISOString().split('T')[0];
            setMinSalida(minDate);

            // Adjust exit date if it's before the new minimum
            if (fechaSalida && new Date(fechaSalida) < new Date(minDate)) {
                setFechaSalida('');
            }
        }
    }, [fechaEntrada]);
